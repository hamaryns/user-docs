...
aufladen: siehe Bankkonto mit Foodsoft verknüpfen.
...
-   Gewichtsabweichungen einer Bestellung: bestellt wurde z.B. ein Stück
    ...
    > Gewicht, da jeder Artikel ein individuelles Gewicht und damit
    > einen individuellen Preis hat. Verrechnet bzw. gutgeschrieben
    > werden nur die Differenzen zum Preis, der in der Foodsoft
    > hinterlegt ist, Beispiel für 500 g und 10€/kg, also Foodsoft Preis
    > 5,00 €:
...
> Eine Funktion der Foodsoft, die es ermöglicht, Gewichtsabweichungen
> auf direktere Art abzurechnen, ist in Planung (Stand Juli 2020).
...
-   Foodcoop Transaktion erstellen: Die Summe aller eingegeben Beträge
    ...
    > gegengebucht (siehe Foodsoft-Kontoauszüge anzeigen).
...
-   Der Kontostand aller Transaktionsklassen wirkt sich grundsätzlich
    ...
    > Transaktionsklasse der Mitgliedsbeiträge aktiviert werden (neu
    > März 2021).
...
[[Kontotransaktion Klassen und
Typen]{.underline}](#transaktionsklassen-und-transaktionstypen) mit
zumindest einem Kürzel eines Transaktionstyps angelegt wurden,
mindestens ein Bankkonto hinzugefügt wurde, und du danach aus der
Foodsoft aussteigst und wieder neu einloggst. Der
Zahlungsreferenz-Rechner verwendet die Kürzel der Transaktionstypen für
die Generierung des Codes, indem für jedes Kürzel ein Feld für einen
Geldbetrag angezeigt wird.
...
1.  Finanzen \> Bankkonten \> Importieren:\
    ...
    ![](media/image6.png){width="1.925in"
...
3.  Neue Kontozeilen werden in die Foodsoft importiert. Die
    ...
    ![](media/image7.png){width="0.75in" height="0.28125in"}
...
### Transaktionen zuordnen: automatische Erstellung eines Finanzlinks, Guthaben aufladen, Rechnungen als bezahlt kennzeichnen
...
Papier-Zahlschein, \...) Zahlungsreferenzcode vorhanden ist, muss
manuell ein Finanzlink erstellt und eine Foodsoft-Kontotransaktion
(Aufbuchen des Guthabens auf das entsprechende
Foodosoft-Bestellgruppenkonto) erstellt werden. Ebenso kann bei
Rechnungen z.B. durch abweichende Rechnungsbeträge, abweichende IBAN
oder abweichende Zahlungsreferenz die Zuordnung fehlschlagen, sodass ein
Finanzlink manuell erstellt und die Rechnung manuell als bezahlt
markiert werden muss. Außerdem kann ein bestehender Finanzlink auch
erweitert werden, z.B. falls die Zahlung einer Rechnung in mehreren
Überweisungen erfolgte und daher einer Rechnung mehrere
Banktransaktionen zuzuordnen sind.
...
-   **Banktransaktion hinzufügen**: eine weitere Bankkontobuchung
    verknüpfen, z.B. wenn eine Rechnung in mehreren Teilbeträgen bezahlt
    wurde.
...
-   Rechnung wird als bezahlt mit entsprechendem Datum markiert und
    > verschwindet damit aus der Liste Finanzen \> Übersicht \>
...
1.  Anhand der **IBAN** der LieferantIn werden offenen Rechnungen
    > gesucht. Die IBAN muss in der Foodsoft unter Artikel \>
    > Lieferanten bei der jeweiligen Lieferantin unter Bearbeiten im
    > Feld IBAN eingegeben sein.
...
3.  Wenn die Summe der \"passenden\" Rechnungen mit dem
    > **Überweisungsbetrag** übereinstimmt, wird eine Finanzlink
    > angelegt und die Rechnungen als bezahlt markiert.
...
siehe Foodsoft-Kontoauszüge anzeigen
...
Menü „Artikel" wird angezeigt, kann Lieferanten, Lager und Lagerartikel
einsehen und bearbeiten, jedoch nicht die Artikel der Lieferanten
einsehen: Link auf Artikel bei Lieferanten scheint zwar auf, aber beim
Anklicken kommt Hinweis: keine Berechtigung
...
Mitglieder mit Abholtage Berechtigung können im *Menü \> Bestellungen \>
Abholtage*** **wählen. Dort kannst du bei der jeweiligen Bestellung auf
den *Download*-Schalter drücken und zb. *Gruppen
PDF*** **wählen. Freischalten der Funktion Abholtage für eine FoodCoop
(Untermenü Bestellungen)

-   Es gibt unter *Menü \> Administration \> Arbeitsgruppen*** **die
    Möglichkeit einer Arbeitsgruppe die Berechtigung Abholtage** **zu
    erteilen. Bitte beachte dass Berechtigungen nur von Admins vergeben
    werden können.
...
[[https://github.com/foodcoops/foodsoft/issues/825]{.underline}](https://github.com/foodcoops/foodsoft/issues/825)
...
-   Minimaler Kontostand: minimal erforderliches verfügbares
    Guthaben(?), um bestellen zu können; leer = Null
...
-   Zugriff auf: jedes Mitglied der Foodcoop hat automatisch Zugriff auf
    folgende Bereiche. Siehe Berechtigungen
...
-   Bestelltoleranz maximal ausnutzen, um möglichst große Mengen zu
    bestellen
...
[]{#__RefHeading___Toc1326_1824384502 .anchor}Teil 3: Foodsoft
Programmierung
...
-   [[https://github.com/foodcoops/foodsoft]{.underline}](https://github.com/foodcoops/foodsoft)
    -- der Hauptzweig der Foodsoft.

-   [[https://github.com/foodcoopsat/foodsoft]{.underline}](https://github.com/foodcoopsat/foodsoft)
    -- eine Abspaltung („Fork") des Hauptzweigs, der den Stand der
    Foodsoft am IG Foodcoops Server
    ([[https://app.foodcoops.at]{.underline}](https://app.foodcoops.at/)/\...)
...
-   Sobald du dich registrierst und selbst Änderungen durchführst,
    ...
    [[https://github.com/DEIN\_GITHUB\_BENUTZERNAME/foodsoft]{.underline}](https://github.com/DEIN_GITHUB_BENUTZERNAME/foodsoft)
    erreichbar ist.
...
1.  **Docker** Engine installieren:
    [[https://docs.docker.com/engine/install/ubuntu/]{.underline}](https://docs.docker.com/engine/install/ubuntu/)

2.  Docker Compose installieren:
    [[https://docs.docker.com/compose/install/]{.underline}](https://docs.docker.com/compose/install/) \>
...
3.  Docker so konfigurieren, dass es die nötigen Berechtigungen hat:
    [[https://docs.docker.com/engine/install/linux-postinstall/\#manage-docker-as-a-non-root-user]{.underline}](https://docs.docker.com/engine/install/linux-postinstall/#manage-docker-as-a-non-root-user)
...
5.  **Github** installieren mit *sudo apt-get install git*\
    siehe auch
    [[https://git-scm.com/download/linux]{.underline}](https://git-scm.com/download/linux)
...
8.  **Foodsoft** herunterladen mit *git clone
    ...
    [[https://github.com/foodcoops/foodsoft/archive/master.zip]{.underline}](https://github.com/foodcoops/foodsoft/archive/master.zip)~~

    ~~Entpacken: es wird ein Verzeichnis *foodsoft-master* angelegt~~

9.  Docker Compose aus dem Verzeichnis *foodsoft* aufrufen (dort
    befindet sich die Datei *docker-compose-dev.yml*), wie unter
    **Setup** beschrieben:
    [[https://github.com/foodcoops/foodsoft/blob/master/doc/SETUP\_DEVELOPMENT\_DOCKER.md]{.underline}](https://github.com/foodcoops/foodsoft/blob/master/doc/SETUP_DEVELOPMENT_DOCKER.md)
...
10. Falls es beim ersten Mal nach der Installation zu Fehlermeldungen
    kommt, den Computer neu starten und Schritt 1 wiederholen

11. Aufruf der lokalen Instanz im Webbrowser:
    [[http://localhost:3000]{.underline}](http://localhost:3000/)

12. Anmelden mit Benutzer *admin* und Passwort *secret*

### Github mit SSH-Verbindung einrichten

1.  [[https://docs.github.com/en/github/authenticating-to-github/connecting-to-github-with-ssh]{.underline}](https://docs.github.com/en/github/authenticating-to-github/connecting-to-github-with-ssh)

2.  [[https://docs.github.com/en/github/using-git/changing-a-remotes-url\#switching-remote-urls-from-https-to-ssh]{.underline}](https://docs.github.com/en/github/using-git/changing-a-remotes-url#switching-remote-urls-from-https-to-ssh)
...
-   Haupt-System bleibt sauberer („weniger" wird darauf installiert) und
    FS-Umgebung ist isoliert(er) davon
...
-   Installation manuell:
    [[https://github.com/foodcoops/foodsoft/blob/master/doc/SETUP\_DEVELOPMENT.md]{.underline}](https://github.com/foodcoops/foodsoft/blob/master/doc/SETUP_DEVELOPMENT.md)

    -   [[https://forum.foodcoops.at/t/themensammlung-foodsoft/4676/4]{.underline}](https://forum.foodcoops.at/t/themensammlung-foodsoft/4676/4)
        -- Marios Ergänzungen
...
Ruby on Rails (Ruby Framework für Web-Applikationen)
====================================================

Allgemeine Einführungen:

-   [[https://www.tutorialspoint.com/ruby-on-rails/rails-introduction.htm]{.underline}](https://www.tutorialspoint.com/ruby-on-rails/rails-introduction.htm)

-   Ruby in 20 Minutes:
    [[https://www.ruby-lang.org/en/documentation/quickstart/]{.underline}](https://www.ruby-lang.org/en/documentation/quickstart/)

Überblick Foodsoft Dateistruktur
================================
...
-   Übersetzungstexte für deutsche Foodsoft Version bearbeitbar über
    ...
    -   [[https://crowdin.com/translate/foodsoft/]{.underline}](https://crowdin.com/translate/foodsoft/) -
...
sofort im
[[Forum]{.underline}](https://forum.foodcoops.at/c/ag-edv/foodsoft/) in
Bereich AG EDV / Foodsoft diskutiert und auf
[[Github]{.underline}](https://github.com/foodcoops/foodsoft/issues) als
Issues deponiert werden.
...
-   Das aus meiner Sicht drängendste Problem ist das aktuelle
    ...
    > Foodsoft]{.underline}](https://forum.foodcoops.at/t/neue-artikelpreiseingabe-in-der-foodsoft/4312)).
    > Ich bin da mit Menschen vom
    > [[DataFoodConsortium]{.underline}](https://www.datafoodconsortium.org/)
    > in Gespräch, um das ganze auch besser interoperabel zu machen,
    > damit Artikelpreise leichter zwischen unterschiedlichen Systemen
